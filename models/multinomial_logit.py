...
*identical* 11-feature vector (:func:`models._shared.build_feature_vector`
...
- **Reference-class (baseline-category) convention.** Class 0
  ...
  a 3-vector of intercepts and a ``(3, 11)`` coefficient matrix, row
  ...
  multinomial parameterization; it has ``3 * 11 + 3 = 36`` free
  parameters versus the ordinal arm's ``11 + 3 = 14`` — over twice as
...
- **Same Armijo machinery, independently implemented.** The optimizer
  ...
  shape (``intercepts (3,)`` + ``coefficients (3, 11)``) and loss
...
class MultinomialLogitModel:
    ...
    via the coefficient report. ``coefficients`` is a ``(3, 11)``
    ...
    Attributes:
        coefficients: The ``(3, 11)`` matrix of free-class
            coefficients.
...
def to_dict(model: MultinomialLogitModel) -> dict:
    ...
    ``feature_names``, ``coefficients`` (a nested ``3 x 11`` list via
...
def from_dict(d: dict) -> MultinomialLogitModel:
    ...
    3 * 11 entries, intercepts/means/stds must line up with
