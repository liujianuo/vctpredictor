...
identical M13-M17 13-feature vector and identical M10 train/test split
...
- **Target and sign convention (must never be inverted).** The binary
  ...
  xs_i . beta`` — one scalar intercept ``alpha`` plus a 13-vector
...
- **Initialization.** ``beta = zeros(13)``; ``alpha =
...
class BinaryLogitModel:
    ...
    Attributes:
        ...
        coefficients: The 13-vector of fitted coefficients ``beta``.
