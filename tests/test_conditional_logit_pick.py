"""Tests for the conditional-logit pick model (M28).

Covers the 5-feature pick vector builder (hand-computed values on a
small synthetic league — the identical recipes the ban model's own
test fixture verifies, reused unchanged per the M28 plan's "feature
set may be reused" decision), the softmax scoring path (a hand-built
model must return exactly ``softmax(feature . beta)`` and the
...
the picked map is always the highest-``acting_map_win_rate`` map must
fit a positive coefficient on that feature — the mirror image of the
ban model's "banned map always lowest, coefficient negative" test),
the ``to_dict``/``from_dict`` round-trip and JSON serializability, the
opponent resolution's zero/ambiguous ``ValueError`` paths, the wrapped
predictor's ``action != "pick"`` rejection, and the ``fit`` input
validation guards.
"""
...
def _pick_league_tables():
    """Build the hand-computable pick-feature league.

    The identical league the ban model's own test fixture builds (the
    M28 plan's "feature set may be reused" decision means the same
    hand-computed feature values must hold for the pick vector). Team
    ``A`` plays Haven twice (1 win) and Split twice (0 wins): overall 4
    games / 1 win -> prior 0.25, Haven mean ``(1 + 10*0.25) / 12 =
    3.5/12``, Split mean ``2.5/12``. Team ``B`` plays Ascent twice (2
    wins) and Sunset twice (1 win): overall ``0.75``, and full
    shrinkage to ``0.75`` on any map it has never played (e.g. Haven).
    All eight matches are dated before :data:`QUERY_DATE`; a ninth
    match (m9, A vs B on Haven at exactly the query cutoff) is excluded
    from every as-of feature by the strict-``<`` boundary and exists so
    the opponent-resolution positive path has a live match to resolve.
...
def _small_fitted_model():
    ...
    Returns:
        A :class:`ConditionalLogitPickModel` fit on 80 synthetic groups
...
def _pick_loss(Xs, bounds, ys, beta, l2_lambda):
    """Evaluate the ragged-group objective at one parameter point.

    A thin test-side wrapper over the module's own
    :func:`models.conditional_logit_pick._loss_and_gradient` returning
    ...
    Raises:
        ValueError: If shapes are inconsistent (propagated from
            :func:`models.conditional_logit_pick._loss_and_gradient`).
    """
    from models.conditional_logit_pick import _loss_and_gradient
...
def _numeric_gradient(Xs, bounds, ys, beta, l2_lambda, eps):
    ...
    objective via :func:`_pick_loss`, and takes the centered
    difference. The objective is the same clipped-NLL-plus-L2 function
    the analytic gradient differentiates, so at points where the clip
    is inactive (all test points) the two must agree to the
    ...
    for i in range(len(beta_arr)):
        ...
        minus[i] -= eps
        grad[i] = (
            _pick_loss(Xs, bounds, ys, plus, l2_lambda)
            - _pick_loss(Xs, bounds, ys, minus, l2_lambda)
        ) / (2.0 * eps)
    return grad


def _separable_pick_dataset(rng, n_groups, n_maps):
    """Build the separable synthetic pick dataset for the fit regression.

    Constructs ``n_groups`` pick steps, each over ``n_maps`` candidate
    rows: every row is noise except feature 0 (``acting_map_win_rate``),
    which is fixed at ``1.0, 0.8, 0.6, 0.4, 0.2`` plus tiny noise; the
    picked map of each group is always the row with the **highest**
    feature 0. Because standardization is a per-column monotone
    transform, the highest raw feature 0 row stays the highest
    standardized feature 0 row, so a correctly-fitting model must learn
    a *positive* coefficient on feature 0 (higher acting win rate ->
    more pickable) — the mirror image of the ban model's separable
    dataset (banned map always lowest, negative coefficient).

    Args:
        rng: A seeded ``numpy.random.Generator``.
        n_groups: The number of pick steps to generate.
    ...
    for _ in range(n_groups):
        ...
        rows.append(xs)
        ys.append(int(np.argmax(xs[:, 0])))
        bounds.append(bounds[-1] + n_maps)
    return np.vstack(rows), np.asarray(bounds), np.asarray(ys, dtype=int)


# --------------------------------------------------------------------------
# plan#1k: build_pick_feature_vector hand-computed values
# --------------------------------------------------------------------------


def test_build_pick_feature_vector_hand_computed():
    # The full 5-vector at QUERY_DATE, every entry hand-computed from
    # the fixture league (identical to the ban model's own hand-computed
    # values — the feature recipes are reused unchanged):
    ...
    matches_df, maps_df = _pick_league_tables()
    vec = build_pick_feature_vector("A", "B", "Haven", QUERY_DATE, matches_df, maps_df)
...
def test_build_pick_feature_vector_second_map_crosscheck():
    ...
    matches_df, maps_df = _pick_league_tables()
    vec = build_pick_feature_vector("A", "B", "Split", QUERY_DATE, matches_df, maps_df)
...
def test_pick_feature_vector_excludes_queried_match():
...
# --------------------------------------------------------------------------
# plan#1k: softmax / scoring path
# --------------------------------------------------------------------------


def test_predict_pick_distribution_equals_softmax_of_scores():
    ...
    probs = predict_pick_distribution(
        "A", "B", pool, QUERY_DATE, matches_df, maps_df, model
    )
    raw = [
        build_pick_feature_vector("A", "B", name, QUERY_DATE, matches_df, maps_df)
        for name in pool
    ]
...
def test_predict_pick_distribution_rejects_empty_remaining_maps():
    ...
    with pytest.raises(ValueError, match="at least one remaining map"):
        predict_pick_distribution("A", "B", [], QUERY_DATE, matches_df, maps_df, model)
...
def test_softmax_internally_consistent_across_shrinking_candidate_sets():
    ...
    probs_full = predict_pick_distribution(
        "A", "B", full_pool, QUERY_DATE, matches_df, maps_df, model
    )
    probs_subset = predict_pick_distribution(
...
# --------------------------------------------------------------------------
# plan#1d/1k: analytic vs finite-difference gradients
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# plan#1k: fit convergence on separable synthetic data
# --------------------------------------------------------------------------


def test_fit_learns_positive_coefficient_on_picked_strong_map_feature():
    # The sign regression (the mirror image of the ban model's "banned
    # map always lowest, coefficient negative" test): a dataset where
    # the picked map is always the one with the HIGHEST synthetic
    # acting_map_win_rate must fit a POSITIVE coefficient on that
    # feature (higher win rate -> more pickable) — the decision-7
    # intuition "pick my strongest map" made concrete — and it must be
    # the dominant coefficient.
    rng = np.random.default_rng(42)
    X_flat, bounds, ys = _separable_pick_dataset(rng, n_groups=120, n_maps=5)
    model = fit(X_flat, bounds, ys, l2_lambda=0.1, max_iter=2000)
    assert model.coefficients[0] > 0.0
    assert model.coefficients[0] >= np.max(model.coefficients[1:])
...
def test_fit_loss_trace_non_increasing():
    ...
    X_flat, bounds, ys = _separable_pick_dataset(rng, n_groups=80, n_maps=5)
...
# --------------------------------------------------------------------------
# plan#1k: to_dict / from_dict round-trip and serializability
# --------------------------------------------------------------------------
...
def test_to_dict_from_dict_round_trip_and_json_serializable():
    ...
    assert isinstance(restored, ConditionalLogitPickModel)
...
def test_coefficient_report_sorted_by_magnitude_with_directions():
    ...
    # carry the documented sign-derived direction strings (the pick-side
    # "favors more/less pickable" language mirroring the ban model's).
    model = _small_fitted_model()
    ...
    for name, coefficient in zip(FEATURE_NAMES, model.coefficients):
        ...
        if coefficient > 0.0:
            assert entry["direction"] == "favors more pickable"
        elif coefficient < 0.0:
            assert entry["direction"] == "favors less pickable"
        else:
...
# --------------------------------------------------------------------------
# plan#1k: opponent resolution and the wrapped predictor
# --------------------------------------------------------------------------
...
def test_resolve_opponent_id_returns_other_side():
    ...
    from models.conditional_logit_pick import _resolve_opponent_id

    matches_df, _ = _pick_league_tables()
...
def test_predictor_fn_rejects_non_pick_action():
    # The wrapped predictor only supports action == "pick" (the decider
    # step is forced, not chosen, and excluded from the pick
    # likelihood); anything else raises with the action named.
    matches_df, maps_df = _pick_league_tables()
    model = _small_fitted_model()
    fn = make_veto_step_predictor_fn(model)
    with pytest.raises(ValueError, match="only supports action 'pick'"):
        fn("A", "ban", ["Haven", "Split"], QUERY_DATE, matches_df, maps_df)


def test_predictor_fn_closure_matches_direct_predict():
    # The returned closure must (a) produce a length-K simplex for a
    # pick over K remaining maps and (b) agree with the pure path
    # predict_pick_distribution called with the opponent the closure
    ...
    probs = fn("A", "pick", pool, QUERY_DATE, matches_df, maps_df)
    assert len(probs) == len(pool)
    assert sum(probs) == pytest.approx(1.0)
    assert all(p >= 0.0 for p in probs)
    direct = predict_pick_distribution(
        "A", "B", pool, QUERY_DATE, matches_df, maps_df, model
    )
