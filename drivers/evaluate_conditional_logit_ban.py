"""Command-line evaluation of the conditional-logit ban model (roadmap M27).
...
model artifact (``conditional_logit_ban_model.json`` via
:func:`models.conditional_logit_ban.from_dict`) plus the four input
...
``actions_to_score={"ban"}`` filter — the fitted conditional-logit
predictor (:func:`models.conditional_logit_ban.make_veto_step_predictor_fn`),
the M25 greedy arm
...
``data/<version>/conditional_logit_ban_evaluation_report.json``.

**Prerequisite:** ``drivers/train_conditional_logit_ban.py`` must
...
training driver first" signal. This is exactly the shape of
prerequisite distinction ``drivers/evaluate_temperature_calibration.py``
already draws against ``drivers/evaluate.py`` — which is why this is a
separate driver from ``drivers/evaluate_veto.py`` rather than a flag
added to it.

**Why ``actions_to_score={"ban"}.``** The conditional-logit predictor
only supports ``action == "ban"`` (decision 7), but the shared
teacher-forced replay must walk the *full* held-out sequence (decider
and pick steps included) so the ``remaining``-set bookkeeping stays
correct across pick steps too. The ``actions_to_score`` filter scores
only ban steps while still consuming every step's bookkeeping — the
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse the evaluate_conditional_logit_ban.py command line.
    ...
    Returns:
        ...
        ``<output_dir>/<version>/conditional_logit_ban_evaluation_report.json``.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the M27 ban-model evaluation: score the fitted "
            "conditional-logit ban model, the M25 greedy simulator and "
            "the most-frequently-played-map baseline per-step over the "
            "held-out test split's real veto logs (ban actions only), "
            "compare mean cross-entropy / top-1 / top-3 accuracy, and "
            "write conditional_logit_ban_evaluation_report.json."
        )
    )
    parser.add_argument(
        "--version",
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the conditional-logit ban evaluation end to end.

    Logging is configured first so the summary line is visible from the
    CLI. The fitted model artifact is loaded first
    (:func:`models.conditional_logit_ban.from_dict` on
    ``conditional_logit_ban_model.json`` — a missing artifact raises
    ...
    ``actions_to_score={"ban"}`` (the fitted
    :func:`models.conditional_logit_ban.make_veto_step_predictor_fn`
    ...
    ``drivers/evaluate_veto.py`` scores, restricted to ban steps so
    the comparison is apples-to-apples against the ban-only trained
    ...
    Raises:
        ...
        ValueError: If the test split's held-out veto set is empty,
            contains no ban rows, an abbreviation fails reconciliation,
            a match's real veto map-name set mismatches its era pool, a
            predictor output is not a valid distribution (including the
            conditional-logit predictor's ``action != "ban"`` guard and
            its unresolvable/ambiguous-opponent guard), a metric cannot
            be computed, or the three scored tables are not row-aligned
        ...
        KeyError: If any input table or the model artifact lacks a
            ...
            :func:`models.conditional_logit_ban.from_dict`).
    ...
    model_path = (
        output_dir / args.version / "conditional_logit_ban_model.json"
    )
    with open(model_path, encoding="utf-8") as handle:
        model = conditional_logit_ban.from_dict(json.load(handle))
    ...
    # multi-arm report. actions_to_score={"ban"} scores only ban steps
    # while the shared replay still consumes every step's `remaining`
    # bookkeeping (decision 7's filter).
    ...
    scored_conditional = veto_evaluation.score_veto_steps(
        conditional_logit_ban.make_veto_step_predictor_fn(model),
        held_out,
        matches_df,
        maps_df,
    ...
    report = veto_evaluation.build_veto_multi_arm_report(
        {
            "conditional_logit_ban": scored_conditional,
    ...
    artifact_path = (
        output_dir
        / args.version
        / "conditional_logit_ban_evaluation_report.json"
    )
    ...
    logger.info(
        "conditional-logit ban evaluation on %d held-out test-split ban "
        "steps (%s/%s): conditional_logit_ban mean_cross_entropy=%.6f "
        ...
        "top3=%+.4f (conditional_logit_ban), %+.6f/%+.4f/%+.4f (greedy)",
        report["conditional_logit_ban"]["n_steps"],
        output_dir,
        args.version,
        report["conditional_logit_ban"]["mean_cross_entropy"],
        report["conditional_logit_ban"]["top1_accuracy"],
        report["conditional_logit_ban"]["top3_accuracy"],
        ...
        report["deltas_vs_baseline"]["conditional_logit_ban"][
            "mean_cross_entropy"
        ],
        report["deltas_vs_baseline"]["conditional_logit_ban"]["top1_accuracy"],
        report["deltas_vs_baseline"]["conditional_logit_ban"]["top3_accuracy"],
