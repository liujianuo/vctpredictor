"""Command-line training of the ordinal logistic regression model (roadmap M20).

Thin command-line wrapper around :mod:`models.ordinal_logit`, which owns
the pure model logic (feature vector, standardizer, fit, serialization).
This module adds only the CLI/IO glue: argument parsing
...
- ``data/<version>/ordinal_logit_model.json`` — the
  :func:`models.ordinal_logit.to_dict` dict, written with
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse the train_ordinal_logit.py command line.
    ...
    Returns:
        ...
        ``<output_dir>/<version>/ordinal_logit_model.json``. There are
        deliberately no hyperparameter flags: ``l2_lambda``/``max_iter``/
        ``grad_tol``/``loss_tol`` stay as
        :func:`models.ordinal_logit.fit`'s documented defaults, matching
        ``drivers/evaluate.py``'s "no ``--k`` flag" precedent (tuning
        them is future-milestone scope, not a CLI concern).
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train the ordinal logistic regression model over the "
...
def main(argv: Sequence[str] | None = None) -> int:
    """Train the ordinal logistic regression model end to end.
    ...
    training-population means/stds :func:`models.ordinal_logit.predict_proba`
    needs), the artifact is written as
    ``<output_dir>/<version>/ordinal_logit_model.json``, and a one-line
    summary of the fit diagnostics is logged (mirroring
    ``drivers/evaluate.py``'s summary-line convention).
    ...
    artifact_path = (
        output_dir / args.version / "ordinal_logit_model.json"
    )
    artifact_path.parent.mkdir(parents=True, exist_ok=True)
    artifact_path.write_text(
        json.dumps(ordinal_logit.to_dict(model), indent=2, sort_keys=True)
        + "\n",
        encoding="utf-8",
    )

    logger.info(
        "trained ordinal logit on %d maps (%s/%s): converged=%s "
