"""Command-line training of the conditional-logit pick model (roadmap M28).
...
``split="train"``), building the per-step pick training examples
(:func:`evaluation.veto_evaluation.build_pick_training_examples` —
the shared teacher-forced replay filtered to ``action == "pick"``,
which excludes the decider steps from the likelihood by construction),
featurizing each example's candidate maps via
:func:`models.conditional_logit_pick.build_pick_feature_vector` and
flattening them into the ragged-group design matrix
``models.conditional_logit_pick.fit`` consumes (one group per pick
step, ``group_boundaries`` slicing each step's candidate rows out of
the flat matrix, ``y_true_row_index`` holding each step's true-map
row), calling :func:`models.conditional_logit_pick.fit`, and writing
the serialized model artifact
``data/<version>/conditional_logit_pick_model.json``.
...
``models/conditional_logit_pick.py``, which must stay downward-only
...
- ``data/<version>/conditional_logit_pick_model.json`` — the
  :func:`models.conditional_logit_pick.to_dict` dict, written with
...
- ``0`` — always. The hard failures are raises instead, mirroring the
  ...
  input table, an empty train split, a held-out table with no pick
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse the train_conditional_logit_pick.py command line.
    ...
    Returns:
        ...
        :func:`models.conditional_logit_pick.fit`'s documented
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train the conditional-logit pick model over the five "
            "map-varying veto features on the M10 train split's "
            "teacher-forced pick steps and write "
...
def main(argv: Sequence[str] | None = None) -> int:
    """Train the conditional-logit pick model end to end.
    ...
    with ``split="train"``), the pick training examples are built from
    it (:func:`evaluation.veto_evaluation.build_pick_training_examples`
    — the shared teacher-forced replay, pick rows only; decider rows
    are excluded from the likelihood by the ``action == "pick"``
    filter, per decision 14), each example's candidate maps are
    featurized (:func:`models.conditional_logit_pick.build_pick_feature_vector`)
    ...
    model is fit (:func:`models.conditional_logit_pick.fit` — which
    ...
    :func:`models.conditional_logit_pick.predict_pick_distribution`
    needs), the artifact is written as
    ``<output_dir>/<version>/conditional_logit_pick_model.json``, and a
    one-line summary of the fit diagnostics is logged (``n_train`` pick
    ...
    Returns:
        ``0`` always. There is no nonzero exit-code path: the hard
            ...
            held-out table with no pick rows, a feature computation
    ...
    Raises:
        ...
        ValueError: If the train split's held-out veto set is empty,
            contains no pick rows, has a match whose real normalized
            map-name set mismatches its era pool, has a pick row whose
    ...
    examples = veto_evaluation.build_pick_training_examples(
        held_out, matches_df, maps_df
    )

    # Flatten every training pick step's candidate-map feature rows into
    # the one ragged-group design matrix (decision 4): rows in replay
    # order, group s covering the example's len(remaining_maps) rows,
    # y_true_row_index[s] = the true picked map's within-group row.
    ...
    for example in examples:
        for map_name in example.remaining_maps:
            flat_rows.append(
                conditional_logit_pick.build_pick_feature_vector(
    ...
    model = conditional_logit_pick.fit(
        X_flat,
        np.asarray(group_boundaries, dtype=int),
        np.asarray(y_true_row_index, dtype=int),
    )

    artifact_path = output_dir / args.version / "conditional_logit_pick_model.json"
    artifact_path.parent.mkdir(parents=True, exist_ok=True)
    artifact_path.write_text(
        json.dumps(conditional_logit_pick.to_dict(model), indent=2, sort_keys=True)
        + "\n",
        encoding="utf-8",
    )

    logger.info(
        "trained conditional logit pick model on %d train-split pick steps "
