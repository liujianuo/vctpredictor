...
:func:`_league_maps_as_of` (all teams, mirroring ``utils.elo.py``'s
...
- **Recompute margin/OT from raw scores, never join ``labels.parquet``.**
  ...
  This mirrors how ``utils.elo.py`` independently re-derives
...
- **Per-team close-map frequency is a point estimate, unshrunk** (the
  ...
  ``utils.map_win_rate.OverallWinRate``'s ``0.5`` default: a win-rate
...
- **Shrunk OT rate reuses ``utils.map_win_rate``'s shrinkage formula**
...
- **Null-score defensiveness matches ``utils.elo.py``'s fail-loud
...
filter. The date parsing reuses ``utils.asof``'s private helpers
(``_parse_query_date`` / ``_parse_date_column`` / ``_require_columns``)
exactly as ``utils.elo.py`` already does — a documented, already-
precedented ``utils`` -> ``utils`` tradeoff, not a fresh
...
# ``utils.elo.py``'s equivalent, team-id columns are *not* required:
...
def _margin_and_ot(team1_score, team2_score):
    ...
    class task 017's deliverable B fixes in ``utils.elo.py``. Margin is
...
def _league_maps_as_of(
    ...
    Date parsing reuses ``utils.asof``'s private helpers
    (``_parse_query_date`` / ``_parse_date_column`` /
    ``_require_columns``) rather than duplicating them, so the strict-``<``
    boundary, null-date rejection and timezone-naive-only rules are
    byte-for-byte identical to every other as-of consumer. The private
    access is a deliberate, documented choice over a reimplementation
    (the same tradeoff ``utils.elo.py`` already makes).
    ...
    Raises:
        KeyError: If either table lacks a required column (propagated
            from ``utils.asof._require_columns``).
        ValueError: If the filtered ``matches`` frame contains duplicate
            ...
            ``utils.asof._parse_query_date`` /
            ``utils.asof._parse_date_column``).
    ...
    parsed_dates = asof._parse_date_column(matches_df[asof.DATE_COL])
...
def team_close_map_frequency(
    ...
    Raises:
        ...
        TypeError: If the query date is list-like (propagated from
            :func:`utils.asof.maps_as_of`).
    """
    asof._require_columns(maps_df, (TEAM1_SCORE_COL, TEAM2_SCORE_COL), "maps_df")
...
def map_round_margin_variance(
    ...
    ``utils.map_win_rate``'s established convention for this exact
