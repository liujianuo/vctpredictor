...
shrinkage estimator reusing ``utils.map_win_rate``'s arithmetic and
...
and does **not** multiply ``utils.player_form``/``utils.map_win_rate``
...
``_validate_k`` / ``_wins_from_oriented_maps`` from ``utils.map_win_rate``
and ``_build_match_name_lookup`` / ``_chronological_maps`` /
``_validated_roster`` from ``utils.player_form`` — private sibling
helpers, imported rather than copy-pasted so the Beta arithmetic, the
null/tie score guards, and the per-match name-resolution/join stay
defined in exactly one place each. This is the already-precedented
``utils`` -> ``utils`` tradeoff (``utils.elo`` / ``utils.closeness`` do
the same with ``utils.asof``'s private parse helpers), not a driver
import. The win/loss rule is scores-only (``team1_score`` vs
``team2_score`` via the ``team_is_team1`` orientation flag), never the
...
from utils.map_win_rate import _validate_k, _wins_from_oriented_maps
from utils.player_form import (
...
# constant in utils.asof); the rest are M8's matches/maps/
# player_map_stats columns, named here once so functions and tests share
# one spelling.
...
class ShrunkH2H:
    ...
    Mirrors ``utils.map_win_rate.ShrunkWinRate``'s field shape exactly.
...
def _pair_as_of_maps(
    ...
    score columns untouched so :func:`utils.map_win_rate._wins_from_oriented_maps`
...
def team_pair_h2h(
    ...
    Args:
        ...
        k: The shrinkage strength (effective prior sample size); must be
            ...
            :func:`utils.map_win_rate._validate_k`).
    ...
    Raises:
        ValueError: If ``k`` is not a positive finite real number (see
            :func:`utils.map_win_rate._validate_k`); if an as-of map has
            a null/NaN score or tied scores (see
            :func:`utils.map_win_rate._wins_from_oriented_maps`); or if
...
def match_event_stage(match_id: str, matches_df: pd.DataFrame) -> int:
    ...
    Raises:
        KeyError: If ``matches_df`` lacks ``match_id`` or ``event_name``
            (propagated from :func:`utils.asof._require_columns`).
    ...
    """
    asof._require_columns(matches_df, (asof.MATCH_ID_COL, EVENT_NAME_COL), "matches_df")
...
def days_since_last_match(
    ...
    query = asof._parse_query_date(date)
...
def _roster_sets_chronological(
    ...
    Args:
        maps_sorted: The output of
            :func:`utils.player_form._chronological_maps` over the
        ...
        match_names: The ``match_id -> (team1_name, team2_name)`` lookup
            from :func:`utils.player_form._build_match_name_lookup`.
        pms_groups: A ``{(match_id, map_index): group}`` dict of
            ``player_map_stats`` rows, as built in
            :func:`utils.player_form.team_player_form`.
    ...
    Raises:
        ValueError: If a ``player_map_stats`` ``team_name`` matches
            ...
            :func:`utils.player_form._validated_roster`).
    """
...
def team_roster_change(
    ...
    (:func:`utils.player_form._build_match_name_lookup` /
    :func:`utils.player_form._validated_roster`), (3) extracts the
    ...
    Raises:
        ...
        ValueError: If ``jaccard_threshold`` or ``half_life_days`` is
            ...
            :func:`utils.player_form._build_match_name_lookup` — the
            Deliverable-B guard); if a ``player_map_stats`` ``team_name``
            matches neither side of its match (propagated from
            :func:`utils.player_form._validated_roster`); or if the query
    ...
    asof._require_columns(matches_df, (TEAM1_NAME_COL, TEAM2_NAME_COL), "matches_df")
    asof._require_columns(maps_df, (asof.MAP_INDEX_COL,), "maps_df")
...
def load_h2h_context_tables(
    ...
    :func:`utils.player_form.load_player_form_tables`. It reads
