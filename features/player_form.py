...
- **Name/id mismatch -> fail loud (``ValueError``).** If any
  ...
  ``features/map_win_rate.py`` / ``features/elo.py`` /
  ``features/closeness.py``.
...
- **Per-row null ``acs``/``rating`` -> skip-and-count.** A roster row
  ...
  *unweighted* mean for that stat (counted in the stat's
  ...
  guarded loudly in ``features/elo.py`` / ``features/map_win_rate.py``; this
...
2. **Recency weighting across maps, windowed to the last N.** The
   ...
   ``features/elo.py`` established for its league-wide replay) receive
...
see ``features.map_win_rate.OverallWinRate`` for the contrast). With
...
# maps_as_of (shared constant in utils.asof); the name columns and the
# name-resolution/roster helpers live in features._shared (shared with
# features.h2h_context), so they are imported above, not defined here.
ACS_COL = "acs"
RATING_COL = "rating"
